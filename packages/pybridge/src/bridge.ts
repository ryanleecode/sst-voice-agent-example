...
import type { ReceiveType, Type, TypeClass } from '@deepkit/type'
import { ChildProcess, spawn } from 'node:child_process'
import { isAbsolute, join } from 'path'
import { Subject } from 'rxjs'
import { PyBridgeConfig } from './config.js'
import type { Logger as PyBridgeLogger } from './logger.js'
...
function isSubjectType(type: Type): type is TypeClass & { typeArguments: [Type] } {
  return Boolean(
    type.kind === ReflectionKind.class && type.classType === Subject && type.typeArguments,
  )
}
...
export class Controller {
  ...
  messageId: number = 0
  readonly subscribers: { [messageId: number]: (data: RpcMessage) => void } = {}

  constructor(moduleNameOrCode: string, config: PyBridgeConfig, logger: PyBridgeLogger) {
    ...
    logger.log(
      `Start python via ${python} in ${cwd} for ${
        moduleNameOrCode.replace(/\n/g, '\\n').substring(0, 50)
      }`,
    )
    ...
    if (moduleNameOrCode.endsWith('.py')) {
      load = `import sys; sys.path.append('${cwd}'); import ${
        moduleNameOrCode.replace('.py', '')
      } as module;`
    }
...
type PromisifyFn<T extends (...args: any[]) => any> = (
  ...args: Parameters<T>
) => ReturnType<T> extends Subject<infer R> ? ReturnType<T>
...
export class PyBridge {
  ...
  constructor(
    ...
    protected logger: PyBridgeLogger = new Logger([new ConsoleTransport()]),
  ) {}
